...
def create_mask_from_points(points, img_h, img_w):
    ...
    y_coords = torch.arange(img_h, device=device)
    x_coords = torch.arange(img_w, device=device)
    grid_y, grid_x = torch.meshgrid(y_coords, x_coords, indexing='ij')

    p0 = points_tensor
    p1 = torch.roll(points_tensor, shifts=-1, dims=0)
    ...
    y_in_range = (p0y > grid_y) != (p1y > grid_y)

    slope = (p1x - p0x) / (p1y - p0y + 1e-8)
    x_intersect = p0x + (grid_y - p0y) * slope

    x_left = grid_x < x_intersect
    intersect = y_in_range & x_left
    inside_mask = (intersect.sum(dim=-1) % 2) == 1

    mask = (inside_mask * 255).to(torch.uint8).cpu().numpy()
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    in_channels = foreground_img.shape[1] 
    ...
    laplacian_kernel = laplacian_kernel.unsqueeze(0).unsqueeze(0).repeat(in_channels, 1, 1, 1)

    fg_laplacian = F.conv2d(
        foreground_img,
        laplacian_kernel,
        padding=1,
        stride=1,
    ...
    fg_mask_expanded = foreground_mask.expand(-1, in_channels, -1, -1)
    bg_mask_expanded = background_mask.expand(-1, in_channels, -1, -1)

    fg_laplacian_masked = fg_laplacian * fg_mask_expanded
    blended_laplacian_masked = blended_laplacian * bg_mask_expanded

    valid_pixel_num = bg_mask_expanded.sum()
    if valid_pixel_num > 0:
        loss = torch.sum((blended_laplacian_masked - fg_laplacian_masked) ** 2) / valid_pixel_num
    else:
